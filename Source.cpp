...
DWORD writeStringValue(HKEY key, LPCSTR subkey, LPCSTR valueName, LPCSTR value)
{
	...
	HKEY openedKey;
	DWORD result;
	result = RegCreateKeyEx(HKEY_CURRENT_USER, subkey, 0,
		REG_OPTION_NON_VOLATILE, NULL, KEY_ALL_ACCESS, NULL, &openedKey, &disposition);
	if (result != ERROR_SUCCESS)
	{
